...
def get_histos(fout, fileList, variable, rebin=None):
    ...
    for ifile in fileList:
        ...
        ss_id_iso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'SS_id_iso_', variable, rebin)
        os_antiid_antiiso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'OS_antiid_antiiso_', variable, rebin)
        ss_antiid_antiiso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'SS_antiid_antiiso_', variable, rebin)
        os_antiid_iso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'OS_antiid_iso_', variable, rebin)
        ss_antiid_iso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'SS_antiid_iso_', variable, rebin)
        os_id_antiiso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'OS_id_antiiso_', variable, rebin)
        ss_id_antiiso[ifile.split('/')[-1].replace('.root', '')] = grab(fin, 'SS_id_antiiso_', variable, rebin)
        fin.Close()
    return hists, {
        'os_id_iso': os_id_iso,
        'ss_id_iso': ss_id_iso,
