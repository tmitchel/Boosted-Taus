...
class Gen_Factory {
 public:
  ...
  std::shared_ptr<VGen> getJetGenJets() { return std::make_shared<VGen>(jet_genjets); }

 private:
  Int_t nGen, nGoodGen;
  Float_t genMET, genMETPhi;
  VGen gen_particles, gen_jets, jet_genjets;
  TLorentzVector MET_p4;
  std::vector<Int_t> *pids;
  std::vector<Float_t> *pts, *etas, *phis, *masses, *jetPt, *jetEta, *jetPhi, *jetEn, *jetGenPt, *jetGenEta, *jetGenPhi,
      *jetGenEn;
};
...
Gen_Factory::Gen_Factory(TTree *tree)
  ...
  tree->SetBranchAddress("jetGenPt", &jetPt);
  tree->SetBranchAddress("jetGenEta", &jetEta);
  tree->SetBranchAddress("jetGenPhi", &jetPhi);
  tree->SetBranchAddress("jetGenEn", &jetEn);
  tree->SetBranchAddress("jetGenJetPt", &jetGenPt);
  tree->SetBranchAddress("jetGenJetEta", &jetGenEta);
  tree->SetBranchAddress("jetGenJetPhi", &jetGenPhi);
  tree->SetBranchAddress("jetGenJetEn", &jetGenEn);
}
...
void Gen_Factory::Run_Factory() {
  ...
  // sort by pT
  std::sort(gen_particles.begin(), gen_particles.end(),
            [](Gen &p1, Gen &p2) -> bool { return p1.getPt() > p2.getPt(); });
  ...
  for (auto i = 0; i < jetGenPt->size(); i++) {
    jet_genjets.push_back(Gen(jetGenPt->at(i), jetGenEta->at(i), jetGenPhi->at(i), jetGenEn->at(i), true));
  }

  // sort by pT
  std::sort(jet_genjets.begin(), jet_genjets.end(), [](Gen &p1, Gen &p2) -> bool { return p1.getPt() > p2.getPt(); });
